...
const Terminal: React.FC<TerminalProps> = ({ 
  ...
  const [history, setHistory] = useState<{type: 'command' | 'output', content: React.ReactNode}[]>([]);
        ...
        { type: 'output' as const, content: (
          <div className="mb-2">
            ...
            <p className="text-terminal-output">
              Type <span className="text-terminal-amber">'help'</span> to see available commands
            </p>
          </div>
        )},
      ];
  ...
  return (
    <TerminalWindow className={cn(className)} title="dev@portfolio: ~">
      <div ref={terminalBodyRef} className="terminal-body h-full overflow-y-auto">
        {history.map((item, i) => (
          <div key={i} className={`${item.type === 'command' ? 'terminal-prompt' : 'command-output'}`}>
            {item.type === 'command' ? (
